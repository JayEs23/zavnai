      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {filteredCommitments.length === 0 ? (
          ...
          filteredCommitments.map((commitment) => (
            <div
              key={commitment.id}
              className={`border rounded-2xl p-4 transition-all hover:shadow-md ${getStatusColor(
                commitment
              )}`}
            >
              <div className="flex items-start gap-3">
                <div className="mt-0.5">{getStatusIcon(commitment)}</div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <p
                      className={`text-xs ${
                        isOverdue(commitment) ? 'text-destructive font-medium' : 'text-muted-foreground'
                      }`}
                    >
                      {isOverdue(commitment) && 'Overdue: '}
                  ...
                  {/* Verification score */}
                  {commitment.verification_score !== undefined &&
                    commitment.verification_score > 0 && (
                      <div className="mt-2">
                        <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                          <div
                            className="h-full bg-success transition-all"
                            style={{ width: `${commitment.verification_score}%` }}
                          />
                        </div>
                        <p className="text-xs text-muted-foreground mt-1">
                          {commitment.verification_score}% verified
                        </p>
                      </div>
                    )}
